...
class archClass {
    ...
    bind(target, firstrun = false) {
        return bind(this.archetype, target, firstrun);
    };
    build() {
...
const bind = function(archetype, target) {
    ...
    tdfs.forEach(function(tdf) {
        ...
        for (let i = 1; i < tdf.length; i++) {
            if (tdf[i] != '') {
                ...
                switch (type) {
                    case 'COMMON':
                        cVars[key.split('.')[1]] = value;
                        break;
                    case 'VERSION':
                        vVars[key.split('.')[1]] = value;
                        break;
                    case 'DAMAGE':
                        dVars[key.split('.')[1]] = value;
                        break;
                    default:
                        Vars[key] = value;
                        break;
...
exports.create = function(engine, fromversion = false) {
    ...
    if (typeof engine.folders.weapons != 'undefined') {
        exports.weapons.bind(engine.folders.weapons, true);
    }
    if (typeof engine.folders.features != 'undefined') {
        exports.features.bind(engine.folders.features, true);
    }
    if (typeof engine.folders.guis != 'undefined') {
        exports.guis.bind(engine.folders.guis, true);
    }
    if (typeof engine.folders.downloads != 'undefined') {
        exports.downloads.bind(engine.folders.downloads, true);
    }
    return exports;
}
